      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-20 items-start">
          ...
          >
            ...
            <h2 className="text-4xl md:text-5xl font-black text-foreground tracking-tight mb-8 leading-[1.05]">
              Building real things.<br />
              <span className="text-foreground/45 font-normal text-3xl">Since I was six.</span>
            </h2>
            <div className="space-y-5 text-[15px] text-foreground/70 leading-relaxed mb-10">
              ...
              <p>
                I've won <span className="text-foreground font-semibold">15+ international hackathons</span> — ETHKL24, Solana Wormhole, Toyota GR —
                and spent the time between them studying whitepapers, breaking contracts, and shipping real protocols.
              </p>
            </div>
            <blockquote className="border-l-2 border-black/[0.08] pl-5 text-foreground/50 italic text-sm leading-relaxed">
          ...
          <div className="grid grid-cols-2 gap-3">
            {highlights.map((h, i) => (
              <motion.div
                key={i}
                className="glass-card rounded-2xl p-6 flex flex-col gap-1 hover:border-black/[0.12] transition-all duration-300 cursor-default"
                ...
                whileHover={{ y: -2 }}
              >
                <span className="text-xs text-foreground/45 font-medium">{h.label}</span>
                <span className="text-lg font-bold text-foreground leading-tight">{h.value}</span>
                <span className="text-xs text-foreground/35">{h.sub}</span>
              </motion.div>
            ))}
          </div>
